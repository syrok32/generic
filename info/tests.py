...
class CoursTests(APITestCase):
    def setUp(self):
        self.admin_user = User.objects.create_user(username='admin', email='s@.ru', password='123qwe', is_staff=True)
        self.owner_user = User.objects.create_user(username='owner', email='owner', password='ownerpass')

        self.course = Cours.objects.create(title='Test Course', desc='Test Description', user=self.owner_user)
    ...
    def test_subscribe_to_course(self):
        ...
        self.assertTrue(Subscription.objects.filter(user_fk=self.owner_user, cuors_fk=self.course).exists())
    ...
    def test_unsubscribe_from_course(self):
        ...
        self.assertFalse(Subscription.objects.filter(user_fk=self.owner_user, cuors_fk=self.course).exists())
